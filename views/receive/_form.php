...
use app\models\AccountsChecking;
    ...
    <div class="row">
        ...
        <div class="col-xs-6 col-sm-4 col-md-3">
            <?= $form->field($model, 'account_id')->dropDownList(
                ArrayHelper::map(AccountsChecking::find()->all(), 'id', 'name'),  // Flat array ('id'=>'label')
                ['prompt'=>'* Pilih Account *']                          // options
            ); ?>
        </div>
        <div class="col-xs-6 col-sm-4 col-md-3">
            <?= $form->field($model, 'journal_value_real')->textInput(['maxlength' => true])->label('Receive Value (Debet)') ?>
          </div>
    </div>
